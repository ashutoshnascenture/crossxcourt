...
$doDirectPaymentResponse = json_decode($data,true);
//echo '<pre>'; print_r($doDirectPaymentResponse);die;
...
<div class="container">
			...
			<div id="return" class="col-md-12 return-box">
				...
				if ($doDirectPaymentResponse['status'] == 'succeeded') {
						$doDirectPaymentResponse['amount'] = $doDirectPaymentResponse['amount']/100;
					...
					<?php echo "<P> ". Lang::get('paymentResponse.amount') .":<span>". $doDirectPaymentResponse['amount'] . " (" . $doDirectPaymentResponse['currency'] . ") </span></P>";
					echo "<P class='transaction'>". Lang::get('paymentResponse.transactionId') .":<span>". $doDirectPaymentResponse['id'] . "</span></P>";
					
					echo "<br><div class='btn btn-default back_btn'><a  href='".URL::to('/')."' id= 'btn'><< ". Lang::get('paymentResponse.continue') ." </a></div>";
				} elseif($doDirectPaymentResponse['status'] == 'pending') {
					 echo "<h3 id='fail'>". Lang::get('paymentResponse.paymentPending') ."</h3>";
					echo "<p>". Lang::get('paymentResponse.pendingErrorMsg') ."</p>";
