	...
	*/
	...
	{		 
		...
		if(!empty($coach_packages))
			return view('checkout', array('coach_packages' => $coach_packages, 'coach_data' => $coach_data,'rate'=>$rate,'publishable_key'=>$publishable_key));
	    else {
			return redirect()->back();	
		}
	}
	...
	public function doDirectPayment(Request $request){
	    try {
			...
			if (!$token)
      			return redirect()->back()->with('msg', 'The Stripe Token was not generated correctly.s');
			...
			if($currency != 'USD'){
				$package->rate = \Helper::get_price('USD', $currency, $package->rate);
			}
			if($max_students > 0){
			...
			if ($result['status']) {
				...
				Session::put('doDirectPaymentResponse', $result);
	...
	{	
 		try {		
 			require_once base_path().'/vendor/braintree/Braintree.php';
			$environment 	= env('Braintree_Configuration_Environment');
 			$merchantId 	= env('Braintree_Configuration_MerchantId');
 			$publicKey      = env('Braintree_Configuration_PublicKey');
 			$privateKey 	= env('Braintree_Configuration_PrivateKey');
			...
			$expirationDate = $request->expDateMonth.'/'.$request->expDateYear;
			$card_info = array( 
				'number' => $request->creditCardNumber, 
				'cardholderName' => $request->firstName,
				'expirationDate' => $expirationDate,
				'cvv' => $request->cvv2Number
			...
			if ($result->success) {
				...
				//$error.= $result->transaction->processorResponseText;
				//echo "<pre>";print_r($result->transaction->processorResponseText);die;
				return redirect()->back()->with('msg', $result->transaction->processorResponseText);
			}
			 else {
